...
uint8_t K[16] = {
    0x33, 0x36, 0xf4, 0x2c, 0x46, 0x5b, 0x21, 0x11,
    0x01, 0xb8, 0xc3, 0x1a, 0x19, 0x09, 0x1b, 0x88
};
uint16_t EK[16];


const uint8_t S7TABLE[128] = {
...
uint64_t FO(uint64_t fo_in, int k) {
    uint16_t t0, t1;
    uint64_t fo_out;
    t0 = fo_in >> 16;
    t1 = fo_in & 0xffff;
...
uint64_t FL(uint64_t fl_in, int k) {
    uint16_t d0, d1;
    uint64_t fl_out;
    d0 = fl_in >> 16;
    d1 = fl_in & 0xffff;
...
uint64_t flinv(uint64_t fl_in, int k) {
    int d0, d1;
    uint64_t fl_out;
    d0 = fl_in >> 16;
    d1 = fl_in & 0xffff;
...
void key_schedule(uint8_t* K, uint16_t* EK) {
    int size = 8;
...
uint64_t encryptblock(uintptr_t P) {
    uint32_t D0 = P >> 32;
    uint32_t D1 = P & 0xffffffff;
    // 0 round
    D0 = FL(D0, 0);
    D1 = FL(D1, 1);
    D1 = D1 ^ FO(D0, 0);
...
uint64_t decryptblock(uintptr_t C) {
    uint32_t D0 = C & 0xffffffff;
    uint32_t D1 = C >> 32;
    D0 = flinv(D0, 8);
    D1 = flinv(D1, 9);
    D0 = D0 ^ FO(D1, 7);
...
int main()
{
    ...
    input.push_back(IV);
    cipher.push_back(IV);
    input.push_back(0xFC2EEFF4AC226B82);
    input.push_back(0xDF5D0DFC3C1061BD);
    input.push_back(0x445D0D33551061BD);
    cout << "input:\t   ";
    ...
    for (size_t i = 1; i < blocknum; i++) {
        uint64_t currP = input[i] ^ cipher[i - 1];
        cipher.push_back(encryptblock(currP));
        //cout << "P" << i << ": " << hex << (input[i]) << endl;
        //cout << "P^" << i << ": " << hex << currP << endl;
        //cout << "C" << i << ": " << hex << (cipher[i])<<endl;
    }
    cout << endl;
    cout << "encrypted: ";
    for (int i = 1;i < blocknum;i++) {
    ...
    for (size_t i = 1; i < blocknum; i++) {
        ...
        plain.push_back(decryptblock(cipher[i]) ^ cipher[i - 1]);
        //cout << "C" << i << ": " << hex << (cipher[i]) << endl;
        //cout << "De" << i << ": " << hex << decryptblock(cipher[i]) << endl;
        //cout << "P" << i << ": " << hex << (plain[i-1]) << endl;
    }
    cout << endl;
    cout << "decrypted: ";
